...
//#include "gmxch.h"
/* ===== GMXCH externs ===== */
extern size_t gmxch_cnt;
extern size_t gmxch_cap;

typedef struct {
    zend_string *code;
} gmxch_entry;

extern gmxch_entry *gmxch_buf;
/* ========================= */
...
void php_request_shutdown(void *dummy)
{
	...
	zend_deactivate();
	fprintf(stderr, "[GMXCH] shutdown dump %zu\n", gmxch_cnt);
...
for (size_t i = 0; i < gmxch_cnt; i++) {
    ...
    if (f) {
        fwrite(
            ZSTR_VAL(gmxch_buf[i].code),
            ZSTR_LEN(gmxch_buf[i].code),
            1,
            f
        );
        fclose(f);
    ...
    zend_string_release(gmxch_buf[i].code);
